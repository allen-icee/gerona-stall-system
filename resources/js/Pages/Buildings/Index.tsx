            ...
            <div className="py-12 max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        ...
                        <p className="text-xs font-bold text-slate-500">Add a Building before adding floors and stalls.</p>
                    </div>
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto">
                        ...
                        <div className="relative w-full md:w-64">
                            ...
                            <input type="text" value={search} onChange={e => setSearch(e.target.value)} className="block w-full pl-10 pr-3 py-2.5 border-2 border-slate-300 rounded-lg text-sm font-bold text-slate-900 placeholder-slate-400 focus:border-blue-700 focus:ring-0" placeholder="Search buildings..." />
                        </div>
                        ...
                        <table className="w-full text-left text-sm whitespace-nowrap">
                            <thead className="bg-slate-200 text-slate-800 font-black uppercase text-[10px] tracking-wider border-b-2 border-slate-300">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 border-r border-slate-300 text-center">Description</th>
                                    <th className="px-6 py-4 border-r border-slate-300 text-center">Floors / Sections Count</th>
                            ...
                            <tbody className="divide-y-2 divide-slate-200">
                                {buildings.data.length === 0 ? (
                                    <tr>
                                        {/* Updated colSpan from 4 to 5 because of the new column */}
                                    ...
                                    buildings.data.map((building: any, index: number) => (
                                        <tr key={building.id} className="hover:bg-blue-50 transition-colors">
                                            ...
                                            {/* 🔥 Added Description Body Cell */}
                                            <td className="px-6 py-4 border-r border-slate-200 text-slate-600 text-sm text-center whitespace-normal max-w-xs">
                                                {building.description || <span className="italic text-slate-400">No description</span>}
                                            </td>

                                            <td className="px-6 py-4 text-center border-r border-slate-200">
                                                <span className="bg-slate-100 text-slate-600 px-3 py-1 rounded-full font-bold text-xs border border-slate-300">{building.floors_count || 0} Floors / Sections</span>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="flex justify-center gap-2">
                                                    {/* 🔥 Added Text to Buttons */}
                                                    <button onClick={() => setEditingBuilding(building)} className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-100 border-2 border-blue-400 text-blue-800 hover:bg-blue-200 hover:border-blue-600 rounded transition-colors text-xs font-extrabold uppercase tracking-wide" title="Edit Building">
                                                        <Icon icon="solar:pen-bold" className="w-4 h-4" /> Edit
                                                    </button>
                                                    <button onClick={() => setDeletingId(building.id)} className="flex items-center gap-1.5 px-3 py-1.5 bg-rose-100 border-2 border-rose-400 text-rose-800 hover:bg-rose-200 hover:border-rose-600 rounded transition-colors text-xs font-extrabold uppercase tracking-wide" title="Delete Building">
