...
export default function PaymentsIndex({ payments, activeContracts, buildings, filters, stats }: any) {
    const { flash } = usePage().props as any; // 🔥 Grab flash messages for Auto-Print

    const [search, setSearch] = useState(filters?.search || "");
    const [sortFilter, setSortFilter] = useState(filters?.sort ? `${filters.sort}_${filters.direction}` : 'payment_date_desc');
    ...
    const monthOptions = [{ value: "", label: "All Months" }, ...monthList.map(m => ({ value: m, label: m }))];

    const currentYear = new Date().getFullYear();
    const yearOptions = [{ value: "", label: "All Years" }, ...Array.from({ length: 5 }, (_, i) => ({ value: (currentYear - i).toString(), label: (currentYear - i).toString() }))];
    ...
    useEffect(() => {
        if (flash?.recent_payment_id) {
            ...
            setTimeout(() => {
                window.open(route("payments.print", flash.recent_payment_id), "_blank");
            }, 500);
        }
    ...
    useEffect(() => {
        const delay = setTimeout(() => {
            ...
            router.get(route("payments.index"), {
                ...
                building_id: filterBuilding, month: filterMonth, year: filterYear
    ...
    const handleDelete = () => {
        if (deletingId) {
            router.delete(route("payments.destroy", deletingId), { preserveScroll: true, onFinish: () => setDeletingId(null) });
        }
    };
        ...
        <AuthenticatedLayout>
            ...
            <Modal show={deletingId !== null} onClose={() => setDeletingId(null)} maxWidth="sm">
                <div className="p-6 text-center">
                    ...
                    <p className="text-sm text-slate-700 font-medium mb-6">Are you sure you want to completely remove this Official Receipt? This will reverse the payment in the tenant's ledger.</p>
                    <div className="flex justify-center gap-3">
                        ...
                        <button onClick={handleDelete} className="px-4 py-2 bg-rose-600 border-2 border-rose-700 text-white font-bold rounded-lg hover:bg-rose-700">Yes, Void Receipt</button>
                    </div>
                </div>
            </Modal>
            ...
            <div className="py-12 max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                    ...
                    <div>
                        <div className="flex items-center gap-3 mb-1">
                            <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tight flex items-center gap-2">
                                <Icon icon="solar:wallet-bold-duotone" className="w-7 h-7 text-emerald-600" /> Treasury Ledger
                            </h3>
                        </div>
                        <p className="text-sm font-bold text-slate-500">Record payments, issue official receipts, and audit collections.</p>
                    </div>
                    <div className="flex items-center gap-3 w-full md:w-auto">
                        <button onClick={() => setIsCreateOpen(true)} className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 border-2 border-emerald-800 text-white font-black uppercase text-xs tracking-wide px-5 py-2.5 rounded-lg shadow-sm shrink-0 whitespace-nowrap">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    <div className="bg-emerald-50 border-2 border-emerald-200 p-5 rounded-2xl flex items-center gap-4 shadow-sm">
                        <div className="bg-emerald-200 text-emerald-700 p-3 rounded-xl border border-emerald-300"><Icon icon="solar:wad-of-money-bold-duotone" className="w-8 h-8" /></div>
                        <div>
                            <p className="text-[10px] font-black uppercase tracking-widest text-emerald-700 mb-0.5">Today's Collection</p>
                            <p className="text-2xl font-black text-slate-900">₱ {Number(stats.today_collection).toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
                        </div>
                    </div>
                    <div className="bg-blue-50 border-2 border-blue-200 p-5 rounded-2xl flex items-center gap-4 shadow-sm">
                        <div className="bg-blue-200 text-blue-700 p-3 rounded-xl border border-blue-300"><Icon icon="solar:calendar-bold-duotone" className="w-8 h-8" /></div>
                        <div>
                            <p className="text-[10px] font-black uppercase tracking-widest text-blue-700 mb-0.5">Monthly Collection</p>
                            <p className="text-2xl font-black text-slate-900">₱ {Number(stats.month_collection).toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
                        </div>
                    </div>
                    <div className="bg-amber-50 border-2 border-amber-200 p-5 rounded-2xl flex items-center gap-4 shadow-sm">
                        <div className="bg-amber-200 text-amber-700 p-3 rounded-xl border border-amber-300"><Icon icon="solar:file-check-bold-duotone" className="w-8 h-8" /></div>
                        <div>
                            <p className="text-[10px] font-black uppercase tracking-widest text-amber-700 mb-0.5">Total Receipts Issued</p>
                            <p className="text-2xl font-black text-slate-900">{stats.total_ors.toLocaleString()}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3 bg-white p-3 rounded-xl border-2 border-slate-300 shadow-sm">
                    <div className="w-48 z-40"><CustomSelect value={sortFilter} onChange={setSortFilter} options={sortOptions} theme="emerald" /></div>
                    <div className="w-48 z-30"><CustomSelect value={filterBuilding} onChange={setFilterBuilding} options={buildingOptions} placeholder="Filter Building" theme="emerald" /></div>
                    <div className="w-40 z-20"><CustomSelect value={filterMonth} onChange={setFilterMonth} options={monthOptions} placeholder="Filter Month" theme="emerald" /></div>
                    <div className="w-36 z-10"><CustomSelect value={filterYear} onChange={setFilterYear} options={yearOptions} placeholder="Filter Year" theme="emerald" /></div>

                    <div className="relative flex-1 min-w-[200px]">
                        <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <Icon icon="solar:magnifer-bold" className="h-5 w-5 text-slate-400" />
                        </div>
                        <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} className="block w-full pl-10 pr-3 py-2 border-2 border-slate-300 rounded-lg text-sm font-bold text-slate-900 placeholder-slate-400 focus:ring-0 focus:border-emerald-600 transition-colors" placeholder="Search OR Number, Payer..." />
                    </div>

                    <button onClick={handleExport} className="flex items-center justify-center p-2 text-emerald-700 bg-emerald-100 rounded-lg border-2 border-emerald-300 hover:bg-emerald-200 shrink-0" title="Export Filtered Collections">
                ...
                <div className="bg-white border-2 border-slate-300 shadow-sm rounded-xl overflow-hidden flex flex-col">
                    <div className="p-4 border-b-2 border-slate-200 bg-slate-50 flex justify-between items-center">
                        <h4 className="font-black text-slate-700 uppercase tracking-tight text-sm flex items-center gap-2">
                            <Icon icon="solar:history-bold-duotone" className="w-5 h-5 text-slate-500" /> Transaction History
                        </h4>
                    </div>
                        ...
                        <table className="w-full text-left text-sm whitespace-nowrap">
                            <thead className="bg-slate-100 text-slate-600 font-black uppercase text-[10px] tracking-wider border-b-2 border-slate-200">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 border-r border-slate-200 text-center">Payer & Details</th>
                                    <th className="px-6 py-4 border-r border-slate-200 text-center">Amount</th>
                                ...
                                {payments.data.length === 0 ? (
                                    <tr>
                                        <td colSpan={6} className="px-6 py-12 text-center text-slate-400 font-bold">
                                            <Icon icon="solar:wallet-money-broken" className="w-12 h-12 mx-auto mb-2 opacity-50" />
                                    ...
                                    payments.data.map((payment: any) => (
                                        <tr key={payment.id} className="hover:bg-emerald-50/50 transition-colors">
                                            <td className="px-6 py-4 font-black text-amber-700 text-center border-r border-slate-100 bg-amber-50/30">{payment.or_number}</td>
                                            <td className="px-6 py-4 border-r border-slate-100 text-center">
                                                <div className="font-black text-slate-900">{payment.contract?.tenant?.last_name}, {payment.contract?.tenant?.first_name}</div>
                                                <div className="text-xs font-bold text-slate-500 mt-0.5 flex items-center justify-center gap-1">
                                                    <Icon icon="solar:shop-bold" className="text-blue-500" /> {payment.contract?.stall?.stall_code}
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 font-black text-emerald-700 text-center border-r border-slate-100 text-base">₱ {Number(payment.amount).toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                                            <td className="px-6 py-4 text-center border-r border-slate-100">
                                                <span className="bg-slate-200 text-slate-700 px-2 py-1 rounded font-bold text-[10px] uppercase">{payment.month} {payment.year}</span>
                                            </td>
