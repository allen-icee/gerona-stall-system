...
export default function EditPaymentModal({ show, onClose, payment }: any) {
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route("payments.update", payment?.id), { onSuccess: () => closeModal() });
    };
        ...
        <Modal show={show} onClose={closeModal} maxWidth="2xl">
            ...
            <form ref={formRef} onSubmit={submit} className="p-6 space-y-5 bg-white rounded-b-2xl">
                ...
                <div className="bg-slate-50 border-2 border-slate-200 rounded-lg p-4 mb-4">
                    <p className="text-xs font-black text-slate-500 uppercase tracking-wide mb-2">Payer Information (Locked)</p>
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <p className="text-[10px] text-slate-400 uppercase font-bold">Tenant</p>
                            <p className="text-sm font-black text-slate-800">{payment?.contract?.tenant?.first_name} {payment?.contract?.tenant?.last_name}</p>
                        </div>
                        <div>
                            <p className="text-[10px] text-slate-400 uppercase font-bold">Stall Code</p>
                            <p className="text-sm font-black text-slate-800">{payment?.contract?.stall?.stall_code}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5 relative z-20">
                    <div>
                        ...
                        <input type="date" value={data.payment_date} onChange={(e) => setData("payment_date", e.target.value)} className="w-full bg-white border-2 border-slate-300 rounded-lg px-4 py-2.5 text-sm font-bold text-slate-900 focus:border-amber-500 focus:ring-0 transition-colors cursor-pointer" required />
                    </div>
                    <div className="z-30">
                        <label className="text-xs font-black text-slate-800 uppercase tracking-wide mb-1 block cursor-pointer">For Month</label>
                        <CustomSelect value={data.month} onChange={(val: any) => setData("month", val)} options={months.map(m => ({ value: m, label: m }))} placeholder="Select Month" theme="amber" />
                    </div>
                    <div>
                        <label className="text-xs font-black text-slate-800 uppercase tracking-wide mb-1 block cursor-pointer">For Year</label>
                        <input type="number" value={data.year} onChange={(e) => setData("year", parseInt(e.target.value))} className="w-full bg-slate-50 border-2 border-slate-300 rounded-lg px-4 py-2.5 text-sm font-bold text-slate-900 focus:border-amber-500 focus:ring-0 transition-colors" required />
                    </div>
                </div>
                ...
                <div className="flex justify-end gap-3 pt-4 border-t-2 border-slate-100">
                    ...
                    <button type="submit" disabled={processing} className="px-6 py-2.5 bg-amber-500 hover:bg-amber-600 text-white rounded-lg font-black uppercase text-xs disabled:opacity-50 transition-colors shadow-sm cursor-pointer">Update Receipt</button>
                </div>
            </form>
        </Modal>
    );
