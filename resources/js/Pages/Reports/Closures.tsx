...
export default function ClosuresReport({ closures, buildings, stalls, filters }: any) {
    const [search, setSearch] = useState(filters?.search || "");
    const [sortFilter, setSortFilter] = useState(filters?.sort ? `${filters.sort}_${filters.direction}` : 'total_outstanding_desc');
    ...
    useEffect(() => {
        const delay = setTimeout(() => {
            ...
            router.get(route("reports.closures"), {
            ...
            <div className="py-12 max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        ...
                        <p className="text-sm font-bold text-slate-500">Tenants flagged for severe delinquency (&gt;₱10k) or closed business permits.</p>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3 bg-white p-3 rounded-xl border-2 border-red-200 shadow-sm">
                    ...
                    <div className="relative flex-1 min-w-[200px]">
                        ...
                        <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} className="block w-full pl-10 pr-3 py-2 border-2 border-slate-300 rounded-lg text-sm font-bold text-slate-900 placeholder-slate-400 focus:ring-0 focus:border-red-600 transition-colors" placeholder="Search Tenant..." />
                    </div>
                </div>
                        ...
                        <table className="w-full text-left text-sm whitespace-nowrap">
                            <thead className="bg-red-900 text-red-100 font-black uppercase text-[10px] tracking-wider border-b-2 border-red-950">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 border-r border-red-800 text-center">Tenant Name</th>
                                    ...
                                    <th className="px-4 py-3 border-r border-red-800 text-center">Total Debt</th>
                                    <th className="px-4 py-3 text-center">Closure Reason / Trigger</th>
                                </tr>
                            </thead>
                                ...
                                {closures.length === 0 ? (
                                    ...
                                    closures.map((record: any) => (
                                        <tr key={record.id} className="hover:bg-red-50 transition-colors">
                                            ...
                                            <td className="px-4 py-4 font-black text-slate-900 border-r border-red-100 text-center">{record.tenant_name}</td>
                                            <td className="px-4 py-4 text-center border-r border-red-100 font-bold text-blue-700">{record.stall_code}</td>
                                            <td className="px-4 py-4 text-center border-r border-red-100">
                                            ...
                                            <td className="px-4 py-4 text-center border-r border-red-100 font-black text-red-700 text-base">₱ {Number(record.total_outstanding).toLocaleString()}</td>
