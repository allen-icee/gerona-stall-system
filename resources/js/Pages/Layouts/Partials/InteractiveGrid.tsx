...
export default function InteractiveGrid({
    ...
    // State for the Expand Tools
    const [selectedMapStallId, setSelectedMapStallId] = useState<number | null>(null);
    ...
    useEffect(() => {
        ...
        const handleWheel = (e: WheelEvent) => {
            if (e.ctrlKey) {
                e.preventDefault();
                setZoom((z) => e.deltaY < 0 ? Math.min(3, z + 0.1) : Math.max(0.4, z - 0.1));
            }
        };
    ...
    const { hiddenCells, cellSpans } = useMemo(() => {
        ...
        gridCells.forEach((cell: any, i: number) => {
            ...
            const colSpan = isText ? (cell.colSpan || 1) : 1;
            const rowSpan = isText ? (cell.rowSpan || 1) : 1;
            ...
            if (colSpan > 1 || rowSpan > 1) {
                ...
                for (let dr = 0; dr < rowSpan; dr++) {
                    for (let dc = 0; dc < colSpan; dc++) {
                        ...
                        const hiddenR = r + dr;
                        const hiddenC = c + dc;
                        if (hiddenR < gridDims.rows && hiddenC < gridDims.cols) {
    ...
    const isRangeVacant = (rStart: number, rEnd: number, cStart: number, cEnd: number) => {
    ...
    // ----- TEXT EXPAND/SHRINK LOGIC -----
    const handleExpandTextRight = (index: number) => {
        ...
        if (!isRangeVacant(r, r + rowSpan - 1, c + colSpan, c + colSpan)) return alert("Cannot expand: Space is occupied.");
    ...
    const handleShrinkTextRight = (index: number) => {
        const newCells = [...gridCells];
        const cell = { ...newCells[index] };
    ...
    const handleExpandTextDown = (index: number) => {
        const newCells = [...gridCells];
        const cell = { ...newCells[index] };
        const r = Math.floor(index / gridDims.cols);
        const c = index % gridDims.cols;
        ...
        if (!isRangeVacant(r + rowSpan, r + rowSpan, c, c + colSpan - 1)) return alert("Cannot expand: Space is occupied.");
    ...
    const handleShrinkTextDown = (index: number) => {
        const newCells = [...gridCells];
        const cell = { ...newCells[index] };
        ...
        gridCells.forEach((cell: any, index: number) => {
            if (cell.type === "stall" && cell.stall) {
                ...
                if (!map.has(cell.stall.id)) {
                    map.set(cell.stall.id, { stall: cell.stall, minR: r, maxR: r, minC: c, maxC: c });
                } else {
    ...
    const setRange = (rStart: number, rEnd: number, cStart: number, cEnd: number, stall: any) => {
        const newCells = [...gridCells];
        for (let r = rStart; r <= rEnd; r++) {
            for (let c = cStart; c <= cEnd; c++) {
                const idx = r * gridDims.cols + c;
                newCells[idx] = { ...newCells[idx], type: stall ? "stall" : "vacant", stall_id: stall ? stall.id : null, stall };
            }
        }
        setGridCells(newCells);
    };
    ...
    const handleExpandRight = (stallId: number) => {
        ...
        if (!isRangeVacant(s.minR, s.maxR, s.maxC + 1, s.maxC + 1)) return alert("Cannot expand: Space is occupied.");
    ...
    const handleExpandDown = (stallId: number) => {
        ...
        if (!isRangeVacant(s.maxR + 1, s.maxR + 1, s.minC, s.maxC)) return alert("Cannot expand: Space is occupied.");
    ...
    return (
        ...
        >
            ...
            <div className="absolute right-6 top-1/2 -translate-y-1/2 z-50 bg-white/90 backdrop-blur-md p-2 rounded-2xl shadow-2xl border-2 border-slate-200 flex flex-col gap-2 items-center pointer-events-auto max-h-[80vh] overflow-y-auto custom-scrollbar">
                ...
                {paintStatuses.map((status) => (
                    <button
                        key={status.id}
                        onClick={() => {
                            setPaintMode(paintMode?.id === status.id ? null : status);
                            setSelectedMapStallId(null);
                        }}
                        ...
                        style={{ backgroundColor: status.color, ["--tw-ring-color" as any]: status.color }}
            ...
            <div className="absolute bottom-6 left-6 z-50 flex flex-col items-start gap-2 pointer-events-auto">
                {isControlsOpen && (
                    <div className="flex flex-wrap items-center gap-2 p-2 bg-white/90 backdrop-blur-md rounded-2xl shadow-2xl border-2 border-slate-200/80 animate-fade-in-up origin-bottom-left">
                        <div className="flex gap-1.5">
                            <button onClick={onRevert} className="flex flex-col items-center justify-center p-2 w-16 h-14 bg-white rounded-xl shadow-sm border border-slate-200 hover:border-amber-400 text-slate-500 hover:text-amber-600 transition-all cursor-pointer">
                                <Icon icon="solar:history-bold-duotone" className="w-5 h-5" />
                                <span className="text-[8px] font-black uppercase mt-1 text-center leading-tight">Revert</span>
                            </button>
                            <button onClick={onClearAll} className="flex flex-col items-center justify-center p-2 w-16 h-14 bg-white rounded-xl shadow-sm border border-slate-200 hover:border-rose-400 text-slate-500 hover:text-rose-600 transition-all cursor-pointer">
                                <Icon icon="solar:eraser-bold-duotone" className="w-5 h-5" />
                                <span className="text-[8px] font-black uppercase mt-1 text-center leading-tight">Clear All</span>
                            </button>
                        </div>
                    </div>
                )}
                <div className="flex items-center gap-2">
                    ...
                    >
                        ...
                        <span className="text-[11px] font-black uppercase tracking-wider">{isControlsOpen ? "Close Tools" : "Map Options"}</span>
                    </button>
            ...
            <div className="absolute bottom-6 right-6 z-40 flex items-center gap-2 bg-white p-1.5 rounded-xl shadow-xl border-2 border-slate-300 pointer-events-auto">
                <button onClick={() => setZoom((z) => Math.max(0.4, z - 0.1))} className="p-2 bg-slate-100 hover:bg-slate-200 rounded-lg text-slate-700 transition-colors cursor-pointer">
                    <Icon icon="solar:minus-circle-bold-duotone" className="w-6 h-6" />
                </button>
                <span className="w-16 text-center font-black text-slate-800 text-xs uppercase tracking-wider">{Math.round(zoom * 100)}%</span>
            ...
            <div ref={containerRef} className={`w-full h-full p-12 overflow-auto flex transition-all ${paintMode ? "cursor-crosshair" : "cursor-default"}`}>
                <div className="m-auto bg-white p-6 rounded-2xl shadow-2xl border-4 border-slate-300 inline-block transition-all duration-300 ease-out">
                    <div className="relative">
                        ...
                        {/* LAYER 1: BASE GRID - EXPLICITLY MAPPED */}
                        <div
                        ...
                        >
                            ...
                            {Array.from({ length: gridDims.cols }).map((_, c) => (
                                <div key={`col-ctrl-${c}`} className="group relative flex justify-center items-end pb-1" style={{ gridColumn: c + 2, gridRow: 1 }}>
                                    <div className="hidden group-hover:flex items-center justify-center gap-1 bg-white rounded-t-lg shadow-md border border-b-0 border-slate-300 px-1 py-1 absolute bottom-0 z-20">
                                        <button onClick={(e) => { e.stopPropagation(); onInsertCol(c); }} title="Insert Column Before" className="text-blue-500 hover:text-blue-700 bg-blue-50 p-0.5 rounded transition-colors cursor-pointer">
                                            <Icon icon="solar:add-square-bold" className="w-4 h-4" />
                                        </button>
                                        <button onClick={(e) => { e.stopPropagation(); onDeleteCol(c); }} title="Delete Column" className="text-rose-500 hover:text-rose-700 bg-rose-50 p-0.5 rounded transition-colors cursor-pointer">
                            ...
                            <div className="flex items-end justify-start pl-1 pb-1" style={{ gridColumn: gridDims.cols + 2, gridRow: 1 }}>
                                <button onClick={(e) => { e.stopPropagation(); onInsertCol(gridDims.cols); }} className="text-slate-400 hover:text-white hover:bg-blue-500 bg-slate-50 border border-slate-200 shadow-sm rounded p-1 transition-all cursor-pointer" title="Add Column at End">
                                    <Icon icon="solar:add-square-bold" className="w-4 h-4" />
                                </button>
                            </div>
                            ...
                            {Array.from({ length: gridDims.rows }).map((_, r) => (
                                <React.Fragment key={`row-${r}`}>
                                    ...
                                    <div className="group relative flex items-center justify-end pr-1" style={{ gridColumn: 1, gridRow: r + 2 }}>
                                        <div className="hidden group-hover:flex flex-col items-center justify-center gap-1 bg-white rounded-l-lg shadow-md border border-r-0 border-slate-300 px-1 py-1 absolute right-0 z-20">
                                            <button onClick={(e) => { e.stopPropagation(); onInsertRow(r); }} title="Insert Row Above" className="text-blue-500 hover:text-blue-700 bg-blue-50 p-0.5 rounded transition-colors cursor-pointer">
                                                <Icon icon="solar:add-square-bold" className="w-4 h-4" />
                                            </button>
                                            <button onClick={(e) => { e.stopPropagation(); onDeleteRow(r); }} title="Delete Row" className="text-rose-500 hover:text-rose-700 bg-rose-50 p-0.5 rounded transition-colors cursor-pointer">
                                                <Icon icon="solar:trash-bin-trash-bold" className="w-4 h-4" />
                                            </button>
                                        </div>
                                    ...
                                    {gridCells.slice(r * gridDims.cols, (r + 1) * gridDims.cols).map((cell: any, c: number) => {
                                        const globalIndex = r * gridDims.cols + c;
                                        if (hiddenCells.has(globalIndex)) return null;

                                        const spans = cellSpans.get(globalIndex) || { colSpan: 1, rowSpan: 1 };

                                        let cellStyle = "bg-slate-50 border-slate-200 border-dashed hover:bg-slate-100 border-b border-r relative group";
                                        let content: any = "";
                                        const isSelectedText = selectedTextIndex === globalIndex;

                                        if (cell.type === "walkway") {
                                            cellStyle = "bg-slate-300 border-slate-400 border-solid text-slate-600 border-b border-r relative";
                                        } else if (cell.type === "restroom") {
                                            cellStyle = "bg-cyan-100 border-cyan-300 border-solid text-cyan-700 border-b border-r relative";
                                            content = "CR";
                                            ...
                                            content = <Icon icon="solar:double-alt-arrow-up-bold-duotone" className="w-full h-full opacity-30 p-1" />;
                                        } else if (cell.type === "wall") {
                                            cellStyle = "bg-slate-800 border-slate-900 border-solid text-slate-500 shadow-inner border-b border-r relative";
                                        } else if (cell.type === "stall") {
                                            cellStyle = "bg-transparent border-none relative";
                                            content = null;
                                        } else if (cell.type === "text") {
                                            cellStyle = "bg-white/40 backdrop-blur-[2px] border-2 border-indigo-200 border-dashed hover:border-indigo-400 text-indigo-900 font-black flex items-center justify-center z-20 p-2 relative transition-all";
                                            if (isSelectedText) cellStyle += " ring-4 ring-indigo-500 ring-offset-1 bg-indigo-50 shadow-xl z-50";

                                            const minSpan = Math.min(spans.colSpan, spans.rowSpan);
                                            const dynamicFontSize = Math.max(10, 14 * zoom * Math.min(2, minSpan));

                                            content = (
                                                <>
                                                    <span className="drop-shadow-sm text-center break-words w-full leading-tight" style={{ fontSize: `${dynamicFontSize}px` }}>
                                                        {cell.text || "Text"}
                                                    </span>
                                                    ...
                                                    {isSelectedText && (
                                                        <>
                                                            <div className="absolute -right-8 top-1/2 -translate-y-1/2 flex flex-col gap-1 bg-slate-800 p-1 rounded-lg shadow-xl z-[60] pointer-events-auto border border-slate-600">
                                                                <button onClick={(e) => { e.stopPropagation(); handleExpandTextRight(globalIndex); }} title="Expand Text Width" className="bg-slate-700 hover:bg-blue-500 text-white w-6 h-6 flex items-center justify-center rounded cursor-pointer"><Icon icon="solar:add-square-bold" /></button>
                                                                <button onClick={(e) => { e.stopPropagation(); handleShrinkTextRight(globalIndex); }} title="Shrink Text Width" className="bg-slate-700 hover:bg-rose-500 text-white w-6 h-6 flex items-center justify-center rounded cursor-pointer"><Icon icon="solar:minus-square-bold" /></button>
                                                            </div>
                                                            <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 flex gap-1 bg-slate-800 p-1 rounded-lg shadow-xl z-[60] pointer-events-auto border border-slate-600">
                                                                <button onClick={(e) => { e.stopPropagation(); handleExpandTextDown(globalIndex); }} title="Expand Text Height" className="bg-slate-700 hover:bg-blue-500 text-white w-6 h-6 flex items-center justify-center rounded cursor-pointer"><Icon icon="solar:add-square-bold" /></button>
                                                                <button onClick={(e) => { e.stopPropagation(); handleShrinkTextDown(globalIndex); }} title="Shrink Text Height" className="bg-slate-700 hover:bg-rose-500 text-white w-6 h-6 flex items-center justify-center rounded cursor-pointer"><Icon icon="solar:minus-square-bold" /></button>
                                                            </div>
                                                        </>
                                                    )}
                                                </>
                                        ...
                                        return (
                                            <div
                                                ...
                                                onClick={(e) => {
                                                    ...
                                                    if (!paintMode) {
                                                        ...
                                                        if (activeTool === 'text' || (!['stall', 'wall', 'restroom', 'stairs', 'vacant', 'walkway'].includes(activeTool) && cell.type === 'text')) {
                                                            setSelectedTextIndex(globalIndex);
                                                        } else {
                                                            setSelectedTextIndex(null);
                                                        }
                                                    }
                                            ...
                                            >
                                                {content && (typeof content === "string" ? <span className="drop-shadow-md bg-white/50 px-1 rounded truncate max-w-full">{content}</span> : content)}
                                            </div>
                                        );
                                    })}

                                    {/* Right Empty Margin Column */}
                                    <div style={{ gridColumn: gridDims.cols + 2, gridRow: r + 2 }}></div>
                                </React.Fragment>
                            ))}

                            {/* Bottom Add Row Button */}
                            <div className="col-span-full pt-1 flex justify-center pb-2" style={{ gridColumn: `2 / span ${gridDims.cols}`, gridRow: gridDims.rows + 2 }}>
                                <button onClick={(e) => { e.stopPropagation(); onInsertRow(gridDims.rows); }} className="flex items-center gap-1 text-[10px] uppercase font-black text-slate-500 bg-white px-4 py-1.5 border border-slate-200 shadow-sm hover:bg-blue-500 hover:border-blue-600 hover:text-white rounded-full transition-colors cursor-pointer">
                                    <Icon icon="solar:add-square-bold" className="w-4 h-4" /> Add Row
                                </button>
                            </div>
                        </div>

                        {/* LAYER 2: BOUNDING BOX OVERLAYS (THE EXPAND TOOL MAGIC FOR STALLS) */}
                        {placedStalls.map((s) => {
                            ...
                            const dbColor = s.stall.computed_status?.color || "#ffffff";
                            const isSelected = selectedMapStallId === s.stall.id;
                            ...
                            if (searchQuery.trim() !== "") {
                                ...
                                const tenantName = `${s.stall.active_contract?.tenant?.first_name || ""} ${s.stall.active_contract?.tenant?.last_name || ""}`.toLowerCase();
                                const companyName = (s.stall.active_contract?.tenant?.company_name || "").toLowerCase();
                                const stallCode = (s.stall.stall_code || "").toLowerCase();
                                ...
                                <div
                                    ...
                                    onClick={(e) => {
                                        ...
                                        if (activeTool === "vacant") {
                                            ...
                                            onQuickPaint(s.stall.id, paintMode.id);
                                        } else {
                                            setSelectedTextIndex(null);
                                    ...
                                    onMouseEnter={(e) => {
                                        if (!paintMode && !isSelected) setTooltip({ show: true, x: e.clientX, y: e.clientY, data: s.stall });
                                    }}
                                    onMouseMove={(e) => {
                                        if (!paintMode && tooltip.show) setTooltip((prev) => ({ ...prev, x: e.clientX, y: e.clientY }));
                                    }}
                                    onMouseLeave={() => setTooltip({ show: false, x: 0, y: 0, data: null })}
                                    className={overlayStyle}
                                    style={{ top, left, width, height, backgroundColor: dbColor }}
                                >
                                    <span className="font-black text-slate-900 drop-shadow-md truncate px-1" style={{ fontSize: `${Math.max(10, 12 * zoom)}px` }}>
                                        {s.stall.stall_code}
                                    </span>
                                    {s.stall.active_contract?.tenant && (
                                        <span className="text-slate-800 font-bold drop-shadow-md truncate w-full text-center px-1 tracking-tight" style={{ fontSize: `${Math.max(8, 10 * zoom)}px` }}>
                                            {s.stall.active_contract.tenant.last_name}
                                        </span>
                                    )}

                                    {/* THE EXPAND TOOL BUTTONS FOR STALLS */}
                                    {isSelected && !paintMode && (
                                        <>
                                            <div className="absolute -right-8 top-1/2 -translate-y-1/2 flex flex-col gap-1 bg-slate-800 p-1 rounded-lg shadow-xl pointer-events-auto">
                                                <button onClick={(e) => { e.stopPropagation(); handleExpandRight(s.stall.id); }} title="Expand Right" className="bg-slate-700 hover:bg-blue-500 text-white w-6 h-6 flex items-center justify-center rounded cursor-pointer"><Icon icon="solar:add-square-bold" /></button>
                                                <button onClick={(e) => { e.stopPropagation(); handleShrinkRight(s.stall.id); }} title="Shrink Right" className="bg-slate-700 hover:bg-rose-500 text-white w-6 h-6 flex items-center justify-center rounded cursor-pointer"><Icon icon="solar:minus-square-bold" /></button>
                                            </div>
                                            <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 flex gap-1 bg-slate-800 p-1 rounded-lg shadow-xl pointer-events-auto">
                                                <button onClick={(e) => { e.stopPropagation(); handleExpandDown(s.stall.id); }} title="Expand Down" className="bg-slate-700 hover:bg-blue-500 text-white w-6 h-6 flex items-center justify-center rounded cursor-pointer"><Icon icon="solar:add-square-bold" /></button>
                                                <button onClick={(e) => { e.stopPropagation(); handleShrinkDown(s.stall.id); }} title="Shrink Up" className="bg-slate-700 hover:bg-rose-500 text-white w-6 h-6 flex items-center justify-center rounded cursor-pointer"><Icon icon="solar:minus-square-bold" /></button>
                                            </div>
                                            <button onClick={(e) => { e.stopPropagation(); handleRemoveStall(s.stall.id); }} title="Delete from Map" className="absolute -top-3 -right-3 bg-rose-600 hover:bg-rose-500 text-white w-6 h-6 rounded-full shadow-xl flex items-center justify-center border-2 border-white cursor-pointer">
