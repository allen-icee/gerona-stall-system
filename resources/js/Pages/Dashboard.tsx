    ...
    return (
        <AuthenticatedLayout>
            ...
            <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
                            ...
                            <div className="bg-white border-2 border-slate-200 shadow-sm rounded-2xl p-6 flex flex-col">
                                ...
                                <div className="space-y-3 mt-auto">
                                    <div className="flex items-center justify-between text-sm">
                                        <div className="flex items-center gap-2 text-slate-600 font-bold"><div className="w-3 h-3 rounded-full bg-emerald-500"></div> Occupied</div>
                                        <span className="font-black text-slate-900">{stats?.occupied || 0}</span>
                                    </div>
                                    <div className="flex items-center justify-between text-sm">
                                        <div className="flex items-center gap-2 text-slate-600 font-bold"><div className="w-3 h-3 rounded-full bg-rose-500"></div> Vacant</div>
                                        <span className="font-black text-slate-900">{stats?.vacant || 0}</span>
                                    </div>
                                    <div className="flex items-center justify-between text-sm">
                                        <div className="flex items-center gap-2 text-slate-600 font-bold"><div className="w-3 h-3 rounded-full bg-amber-400"></div> Maint/Wait</div>
                                        <span className="font-black text-slate-900">{stats?.maintenance || 0}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="overflow-x-auto custom-scrollbar">
                                <table className="w-full text-center text-sm whitespace-nowrap">
                                    <thead className="bg-slate-100 text-slate-600 font-black uppercase text-[10px] tracking-wider border-b-2 border-slate-200">
                                        <tr>
                                            <th className="px-4 py-3 text-left border-r-2 border-slate-200 sticky left-0 bg-slate-100 z-10">Building Name</th>
                                    ...
                                    <tbody className="divide-y divide-slate-100 font-medium">
                                        {buildingSummary?.map((bldg: any, index: number) => (
                                            <tr key={index} className="hover:bg-slate-50 transition-colors">
                                                <td className="px-4 py-3 text-left font-black text-slate-800 border-r-2 border-slate-200 sticky left-0 bg-white z-10">{bldg.name}</td>
                ...
                {userRole === 'treasury' && (
                    <>
                        {/* Custom Treasury KPI Cards */}
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                            <div className="bg-emerald-50 border-2 border-emerald-200 p-6 rounded-2xl flex items-center gap-4 shadow-sm">
                                <div className="bg-emerald-200 text-emerald-700 p-4 rounded-xl border border-emerald-300"><Icon icon="solar:wad-of-money-bold-duotone" className="w-8 h-8" /></div>
                                <div>
                                    <p className="text-[10px] font-black uppercase tracking-widest text-emerald-700 mb-0.5">Today's Collection</p>
                                    <p className="text-3xl font-black text-slate-900">₱ {Number(stats.today_collection).toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
                                </div>
                            </div>
                            <div className="bg-blue-50 border-2 border-blue-200 p-6 rounded-2xl flex items-center gap-4 shadow-sm">
                                <div className="bg-blue-200 text-blue-700 p-4 rounded-xl border border-blue-300"><Icon icon="solar:calendar-bold-duotone" className="w-8 h-8" /></div>
                                <div>
                                    <p className="text-[10px] font-black uppercase tracking-widest text-blue-700 mb-0.5">Monthly Collection</p>
                                    <p className="text-3xl font-black text-slate-900">₱ {Number(stats.month_collection).toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
                                </div>
                            </div>
                            <div className="bg-amber-50 border-2 border-amber-200 p-6 rounded-2xl flex items-center gap-4 shadow-sm">
                                <div className="bg-amber-200 text-amber-700 p-4 rounded-xl border border-amber-300"><Icon icon="solar:file-check-bold-duotone" className="w-8 h-8" /></div>
                                <div>
                                    <p className="text-[10px] font-black uppercase tracking-widest text-amber-700 mb-0.5">Total Receipts Issued</p>
                                    <p className="text-3xl font-black text-slate-900">{stats.total_ors.toLocaleString()}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                            ...
                            <div className="bg-white border-2 border-slate-200 shadow-sm rounded-2xl p-6 flex flex-col items-center justify-center text-center">
                                <Icon icon="solar:ticket-sale-bold-duotone" className="w-20 h-20 text-emerald-500 mb-4 opacity-50" />
                                <h3 className="font-black text-slate-800 text-xl uppercase tracking-tight mb-2">Quick Actions</h3>
                                <p className="text-sm font-bold text-slate-500 mb-6">Need to issue an official receipt or view the Master Ledger?</p>
                                <a href={route("payments.index")} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-black uppercase text-sm px-5 py-3 rounded-lg shadow-sm transition-colors border-b-4 border-emerald-800 flex justify-center items-center gap-2 mb-3">
                                    <Icon icon="solar:add-square-bold" className="w-5 h-5" /> Issue New O.R.
                                </a>
                                <a href={route("reports.master_ledger")} className="w-full bg-slate-800 hover:bg-slate-700 text-white font-black uppercase text-sm px-5 py-3 rounded-lg shadow-sm transition-colors border-b-4 border-slate-900 flex justify-center items-center gap-2">
                                    <Icon icon="solar:database-bold-duotone" className="w-5 h-5" /> Master Ledger
                                </a>
                            </div>
                        </div>
                    </>
                                ...
                                <div className="overflow-x-auto">
                                    <table className="w-full text-left text-sm">
                                        <thead className="bg-slate-50 text-slate-500 font-black uppercase text-[10px] tracking-wider border-b-2 border-slate-200">
                                            <tr>
                                                <th className="px-6 py-3 border-r border-slate-200">Stall #</th>
                                                <th className="px-6 py-3 border-r border-slate-200">Tenant Name</th>
                                                <th className="px-6 py-3 border-r border-slate-200">Action</th>
                                        ...
                                        <tbody className="divide-y divide-slate-100 font-medium">
                                            {recentActivity?.map((activity: any, index: number) => (
                                                <tr key={index} className="hover:bg-blue-50 transition-colors">
                                                    <td className="px-6 py-4 font-black text-slate-800 border-r border-slate-100">{activity.stall_code}</td>
                                                    <td className="px-6 py-4 text-slate-700 border-r border-slate-100">{activity.tenant_name}</td>
                                                    <td className="px-6 py-4 text-slate-500 border-r border-slate-100">{activity.action}</td>
                            ...
                            <div className="bg-gradient-to-br from-rose-900 to-slate-900 border-2 border-slate-800 shadow-lg rounded-2xl p-6 text-white relative overflow-hidden flex-1">
                                ...
                                <ul className="space-y-5 relative z-10">
                                    {expiringContracts && expiringContracts.length > 0 ? (
                                        ...
                                        <li className="text-sm font-bold text-slate-400 italic">No contracts expiring within 30 days.</li>
                                    )}
                                </ul>
                            </div>
                        </div>
