        ...
        <AuthenticatedLayout>
            ...
            <Modal show={deletingId !== null} onClose={() => setDeletingId(null)} maxWidth="sm">
                <div className="p-6 text-center">
                    ...
                    <h3 className="text-xl font-black text-slate-900 mb-2">Delete Stall?</h3>
                    <p className="text-sm text-slate-700 font-medium mb-6">
                        Are you sure you want to completely remove this stall? Any active contracts linked to it might be affected.
                    </p>
                                ...
                                {stalls.data.length === 0 ? (
                                    ...
                                    stalls.data.map((stall: any, index: number) => {
                                        ...
                                        const statusColors: any = {
                                            ...
                                            "FOR SIGNING": "bg-cyan-100 border-cyan-300 text-cyan-700",
                                            "WAITING FOR BUSINESS PERMIT": "bg-fuchsia-100 border-fuchsia-300 text-fuchsia-700",
                                            "ON PROCESS": "bg-slate-200 border-slate-400 text-slate-700",
                                            "FOR CONFIRMATION": "bg-purple-100 border-purple-300 text-purple-700",
                                            "UNPAID PERMIT": "bg-rose-100 border-rose-300 text-rose-700",
                                            "SIGNED CONTRACT": "bg-blue-100 border-blue-300 text-blue-700",
                                        ...
                                        return (
                                            <tr key={stall.id} className="hover:bg-blue-50 transition-colors">
                                                ...
                                                <td className="px-6 py-4 text-center border-r border-slate-200">
                                                    <span className={`inline-block px-3 py-1 rounded border-2 font-black text-[10px] uppercase tracking-wider ${badgeColor}`}>
                                                        {statusObj.label}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4 text-center border-r border-slate-200">
                                                    <div className="font-bold text-slate-800">{stall.floor?.name || <span className="text-rose-500">No Floor</span>}</div>
                                                    <div className="text-xs text-slate-500">{stall.floor?.building?.name || "No Building"}</div>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <div className="flex justify-center gap-2">
                                                        <button onClick={() => setEditingStall(stall)} className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-100 border-2 border-blue-400 text-blue-800 hover:bg-blue-200 hover:border-blue-600 rounded font-black text-xs uppercase tracking-wide transition-colors">
                                                            <Icon icon="solar:pen-bold" className="w-4 h-4" /> Edit
                                                        </button>
                                                        <button onClick={() => confirmDelete(stall.id)} className="flex items-center gap-1.5 px-3 py-1.5 bg-rose-100 border-2 border-rose-400 text-rose-800 hover:bg-rose-200 hover:border-rose-600 rounded font-black text-xs uppercase tracking-wide transition-colors">
            ...
            <CreateStallModal show={isCreateOpen} onClose={() => setIsCreateOpen(false)} floors={floors} />
            <EditStallModal show={editingStall !== null} onClose={() => setEditingStall(null)} stall={editingStall} floors={floors} />
        </AuthenticatedLayout>
    );
}