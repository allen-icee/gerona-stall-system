...
interface Props { show: boolean; onClose: () => void; roles: Role[]; user: User | null; }
...
export default function EditUserModal({ show, onClose, roles, user }: Props) {
    ...
    useEffect(() => {
        if (user) {
            setData({
                ...
                password: '', password_confirmation: '', role: user.roles[0]?.name || 'staff',
            });
        }
    }, [user]);
    ...
    const isPasswordValid = data.password === "" ? true : (allRequirementsMet && passwordsMatch);

    const isFormValid = data.name.trim() !== "" && nameError === "" &&
        data.username.trim() !== "" && usernameError === "" &&
        emailError === "" && isPasswordValid;
    ...
    const submit = (e: FormEvent) => {
        e.preventDefault();
        if (user) put(route('users.update', user.id), { onSuccess: () => closeAndReset() });
    };
