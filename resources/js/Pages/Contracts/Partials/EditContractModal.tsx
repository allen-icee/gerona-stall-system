...
export default function EditContractModal({ show, onClose, contract }: any) {
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route("contracts.update", contract?.id), { onSuccess: () => closeModal() });
    };
            ...
            <form ref={formRef} onSubmit={submit} className="p-6 space-y-6 bg-white rounded-b-2xl overflow-y-auto max-h-[80vh] custom-scrollbar">
                ...
                <div className="bg-slate-800 border-2 border-slate-900 rounded-xl p-4 shadow-inner text-white">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <p className="text-[10px] text-amber-400 uppercase font-bold">Tenant</p>
                ...
                <div className="bg-amber-50 p-4 rounded-xl border-2 border-amber-200">
                    <h3 className="text-[10px] font-black text-amber-600 uppercase tracking-widest mb-4 flex items-center gap-2">
                        <Icon icon="solar:calendar-date-bold-duotone" className="w-4 h-4" /> Terms & Fees
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="text-xs font-black text-amber-900 uppercase tracking-wide mb-1 block cursor-pointer">Start Date</label>
                            <input type="date" value={data.start_date} onChange={(e) => setData("start_date", e.target.value)} className="w-full bg-white border-2 border-amber-300 rounded-lg px-4 py-2 text-sm font-bold focus:border-amber-600 focus:ring-0" required />
                        </div>
                        <div>
                            <label className="text-xs font-black text-amber-900 uppercase tracking-wide mb-1 block cursor-pointer">End Date</label>
                            <input type="date" value={data.end_date} onChange={(e) => setData("end_date", e.target.value)} className="w-full bg-white border-2 border-amber-300 rounded-lg px-4 py-2 text-sm font-bold focus:border-amber-600 focus:ring-0" required />
                        </div>

                        <div>
                            <div className="flex items-center justify-between mb-1">
                                <label className="text-xs font-black text-amber-900 uppercase tracking-wide cursor-pointer">Monthly Rent (₱)</label>
                                <button type="button" onClick={() => setIsRentLocked(!isRentLocked)} className="text-amber-500 hover:text-amber-700 focus:outline-none transition-colors" title={isRentLocked ? "Unlock to edit" : "Lock price"}>
                                    <Icon icon={isRentLocked ? "solar:lock-password-bold-duotone" : "solar:unlock-bold-duotone"} className="w-4 h-4" />
                                </button>
                            </div>
                            <input type="number" step="0.01" value={data.monthly_rent} onChange={(e) => setData("monthly_rent", e.target.value)} readOnly={isRentLocked} className={`w-full border-2 rounded-lg px-4 py-2 text-sm font-black transition-colors focus:ring-0 ${isRentLocked ? 'bg-amber-100 border-amber-200 text-amber-700 cursor-not-allowed' : 'bg-white border-amber-400 text-amber-900 focus:border-amber-600'}`} required />
                        </div>

                        <div>
                            <label className="text-xs font-black text-amber-900 uppercase tracking-wide mb-1 block cursor-pointer">Security Deposit (Bond)</label>
                            <input type="number" step="0.01" value={data.security_deposit} onChange={(e) => setData("security_deposit", e.target.value)} className="w-full bg-white border-2 border-amber-300 rounded-lg px-4 py-2 text-sm font-black focus:border-amber-600 focus:ring-0" placeholder="₱ 0.00" />
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-emerald-50 p-4 rounded-xl border-2 border-emerald-200">
                    <h3 className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-4 flex items-center gap-2">
                        <Icon icon="solar:wallet-money-bold-duotone" className="w-4 h-4" /> Treasury Data
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="text-xs font-black text-emerald-900 uppercase tracking-wide mb-1 block cursor-pointer">Actual Deposit (Advance Rent)</label>
                        ...
                        <div>
                            <label className="text-xs font-black text-emerald-900 uppercase tracking-wide mb-1 block cursor-pointer">O.R. / Reference #</label>
                ...
                <div className="bg-blue-50 p-4 rounded-xl border-2 border-blue-200">
                    <h3 className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-4 flex items-center gap-2">
                        <Icon icon="solar:folder-with-files-bold-duotone" className="w-4 h-4" /> EEDO Status Update
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="text-xs font-black text-blue-900 uppercase tracking-wide mb-1 block cursor-pointer">Document Status</label>
                        ...
                        <div>
                            <label className="text-xs font-black text-blue-900 uppercase tracking-wide mb-1 block cursor-pointer">Permit Status</label>
                        ...
                        <div className="col-span-1 md:col-span-2">
                            <label className="text-xs font-black text-blue-900 uppercase tracking-wide mb-1 block cursor-pointer">Remarks</label>
                ...
                <div className="flex justify-end gap-3 pt-2 border-t-2 border-slate-100">
                    ...
                    <button type="submit" disabled={processing} className="px-6 py-2.5 bg-amber-500 hover:bg-amber-600 text-white rounded-lg font-black uppercase text-xs disabled:opacity-50 transition-colors shadow-sm">Update Contract</button>
                </div>
            </form>
        </Modal>
    );
